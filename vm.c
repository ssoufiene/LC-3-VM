...
#define OPC(i) ((i)>>12) // macro for extracting the OpCode (shiftting 12 bits to the right to get the 4 most significant bits )
#define DR(i) (((i)>>9)&0x7)
...
typedef void (*op_ex_f)(uint16_t i);// function pointer that takes uint16_t instruction as parameter and returns void .
typedef void (*trp_ex_f)();

enum { trp_offset = 0x20 };
// we have 10 registers each 16 bits //
enum regist { R0 = 0, R1, R2, R3, R4, R5, R6, R7, RPC, RCND, RCNT };
uint16_t reg[RCNT] = {0};



//Defining the main memory//
uint16_t mem[UINT16_MAX] = {0};
uint16_t PC_START = 0x3000;


// fucntion for reading//
...
// a function that extends the sign to make it compatible with the 16 bits format:
static inline uint16_t sext(uint16_t n, int b) { return ((n>>(b-1))&1) ? (n|(0xFFFF << b)) : n; } // if the bth bit of n is 1 (number is negative)
                                        // fill up with 1s the remaining bits (15)
                                    // else return the number as it is


//flages for branching is bigger than another number b. We can calculate their difference, 
//and if the result is a negative number, RCND is 1<<2. Then we can use the br instruction to jump to another instruction
enum flags { FP = 1 << 0, FZ = 1 << 1, FN = 1 << 2 };
static inline void uf(enum regist r) {
    if (reg[r]==0) reg[RCND] = FZ; // diff is zero 
    else if (reg[r]>>15) reg[RCND] = FN; // diff is negative number
    else reg[RCND] = FP; // diff is positve 
}
// instructions
